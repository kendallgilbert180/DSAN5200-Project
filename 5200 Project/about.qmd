...
How can we quantify the wellbeing of the individual in various countries? Are some countries more prosperous than others? Can they even be compared? All too often, data scientists fall into the trap of Operationalization. By mis-equating metrics with reality, it is easy to oversimplify extremely complicated and nuanced situations. For example, it is often claimed that policing makes communities safer - but how exactly can we measure safety? At the national scale, this becomes even more complicated. There are countless potential avenues of quantification; yet it is all too easy to reduce the country's prosperity to the classical metric: GDP per capita.

In this exploration, we will conduct a preliminary investigation into other potential ways to quantify a country's well being. Data from the OECD website will be used to scrutinize factors from several distinct buckets such as healthcare, life expectancy, social spending and wealth distribution. Specific metrics from each of these categories will be compared with GDP per capita to assess their relationships. These representative quantifiers will then act as placeholders in a prototype model for a more holistic exploration of a country’s overall wellbeing. Our project effectively acts as a proposal for further research into some such metric in the future. Our single datasets in each field are merely representative, and we hope many different factors can be taken into account in each bucket to create more comprehensive summaries. Additionally, further categories could also be explored using a similar framework to the prototype proposed in the concluding section. 
...
```{python echo=FALSE}
#| warning: false
#| fig-cap: "Figure 5: Life Expectancy by Country"
...
def update_plot(year):
    ...
    fig.update_layout(
        ...
        xaxis_title='Country (Sorted by GDP Per Capita)',
...
for year in years:
    ...
    fig.add_trace(go.Scatter(
        ...
        y=year_df['Moving Average'],
        mode='lines',
        name=f'{year} Life Expectancy Moving Average (5 Countries)',
        line=dict(color='red', width=2),
        hovertemplate='<b>Country:</b> %{x}<br>' +
                      '<b>Year:</b> %{customdata[0]}<br>' +
                      '<b>5 Country Moving Average LE:</b> %{y}<br>' +
...
```{python echo=FALSE}
#| warning: false
#| fig-cap: "Figure 6: Share of Individuals with net Wealth less than 25% of the poverty line"
...
gdp = pd.read_csv('Data_cleaned/gdppc.csv')
...
#sns.scatterplot(data=vis, x="GDP Per Capita (USD)", y="Share of Individuals with net Wealth less than 25% of the poverty line", hue="Country")
#sns.set_theme()
...
ax2 = ax1.twinx()
ax2.plot(vis['Country'], vis['GDP Per Capita (USD)'], color='#ff6f69', marker='o', linestyle='-')  

ax1.set_xlabel('Country', fontproperties=font_prop)
ax1.set_ylabel('Share of Individuals with net Wealth < 25% of Poverty Line',fontproperties=font_prop)
ax2.set_ylabel('GDP per Capita (USD)', color='#ff6f69',fontproperties=font_prop)
ax1.set_facecolor('#faf0e6')

ax1.set_xticklabels(vis['Country'], rotation=45, ha='right', fontproperties=font_prop)

plt.title('Extreme Poverty and its Relationship to GDP per Capita', fontproperties=font_prop)
plt.tight_layout()
plt.show()
```
